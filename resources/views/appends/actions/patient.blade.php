		...
		<div class="dropdown">
			...
			<div class="dropdown-menu" aria-labelledby="formsList{{ $row->id }}">
				@if(Auth::user()->user_type == 2)
					<a href="{{ route('edit-patient', $row->id) }}" class="dropdown-item text-primary">
						<i class="fa fa-pencil" aria-hidden="true"></i> Edit
					</a>

					@if($row->user->status === '1')
                        <a href="{{ route('patient.deactivate', $row->user->id) }}" class="dropdown-item text-danger" title="Deactive">
                            <i class="fa fa-ban" aria-hidden="true"></i> Deactivate
                        </a>
                    @else
                        <a href="{{ route('patient.activate', $row->user->id) }}" class="dropdown-item text-success" title="Active">
